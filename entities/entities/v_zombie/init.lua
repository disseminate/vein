...
function ENT:Initialize()
	...
	self:SetModel( table.Random( V.ZombieModels ) );
...
function ENT:Damage( ply, bite )
	...
	if( ply.Asleep ) then
		...
		V.STAT.AddStatus( ply, V.STAT.STATUS_SLEEP, 1 );
		V.STAT.SubStatus( ply, V.STAT.STATUS_HUNGER, 1 );
		
		net.Start( "nStopSleepPanic" );
			net.WriteEntity( ply );
		net.Broadcast();
	...
	local infected = bite or ( math.random( 1, 20 ) == 1 ); -- :(
	
	if( infected and V.GetCharTime( ply ) >= V.Config.CharacterSafeTime ) then
...
function ENT:OnTakeDamage( dmg )
	...
	if( type == DMG_CRUSH and table.HasValue( V.InstantCrushDeath, inf:GetClass() ) ) then
...
function ENT:MaxEnemyLOSDistance()
	...
	local d = V.Config.ZombieSeeDistance;
	
	if( V.W.Raining and V.Outside( self:EyePos() ) ) then d = d / 2; end
...
function ENT:MaxEnemySoundDistance()
	...
	local d = V.Config.ZombieHearDistance;
	
	if( V.W.Raining and V.Outside( self:EyePos() ) ) then d = d / 3; end
...
function ENT:LostPlayer()
	...
	return ( CurTime() - self.LastSeenEnemy > V.Config.ZombieBoredTime );
...
function ENT:Die()
	...
	for _, v in pairs( player.GetAll() ) do
		...
		if( v.MyZombie == self and V.GetCharTime( v ) >= V.Config.CharacterSafeTime ) then
			
			V.SQL.SaveCharacterField( v, "Dead", "2" );
			V.SQL.SaveCharacterField( v, "DeadPos", tostring( self:GetPos() ) );
